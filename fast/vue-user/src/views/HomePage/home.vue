    ...
    <div class="main-layout">
      ...
      <div class="content-right">
        <div class="ranking-board">
          <h3>Trending Now <span class="dot">·</span> 热销榜</h3>
          <ul class="ranking-list">
            <li v-for="(book, index) in topFiveBookList" :key="index">
              <span class="rank-num" :class="{ 'top-3': index < 3 }">{{ index + 1 }}</span>
              <div class="rank-info">
                <h4>{{ book.title }}</h4>
                <p>已卖出: {{ book.total }}本</p>
              </div>
            </li>
          </ul>
        </div>
...
// 导入API函数
import {listBook, selectRecommendBook, selectTopFiveBooks} from "@/api/mall/book.js";
...
onMounted(() => {
  ...
  //查询销售数量前5的图书以及销售额
  selectTopFiveBooks().then(res => {
    topFiveBookList.value = res.data
    console.log(topFiveBookList.value)
  })
})
</script>
